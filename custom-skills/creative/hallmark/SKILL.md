---
name: hallmark
description: "Use when designing, auditing, redesigning, or studying UI/landing pages with an anti-AI-slop design discipline: structural variety, macrostructures, typography, motion, responsive checks, slop-test gates, and DESIGN.md handoff."
version: 0.1.0
author: Hallmark contributors / adapted for Hermes Agent
license: MIT
metadata:
  hermes:
    tags: [ui, ux, design, frontend, anti-slop, landing-pages]
    related_skills: [taste-skill, ui-skills-root, ui-ux-pro-max, design-md, awesome-design-md-library, improve-ui, fixing-accessibility, fixing-motion-performance]
    source: https://github.com/Nutlope/hallmark
    source_commit: aeb42fb
    local_category: Hallmark Design Skill
---
...
- Source: `Nutlope/hallmark`, commit `aeb42fb`.
- Local category: Hallmark Design Skill / Creative.
- This is integrated as one Hermes skill plus a full local reference library under `references/`.
- Original upstream `SKILL.md` is preserved unchanged at `references/original-SKILL.md`.
- Do not run the upstream Node-based skill installer, npm installs, or external image/API workflows automatically on Termux.
- Use local Hermes tools and existing project files first. Ask before installing Node packages, invoking paid image services, or making destructive redesign edits.
- Load/read only the needed reference files for the current UI task; do not dump the whole library into context.

## Local Reference Library Map

Use these references on demand:

- `references/anti-patterns.md` — AI-slop tells and forbidden patterns.
- `references/slop-test.md` — final quality gates before delivering UI.
- `references/structure.md` and `references/macrostructures/` — structural variety and page fingerprints.
- `references/themes/` and `references/genres/` — theme/genre selection.
- `references/typography.md`, `references/color.md`, `references/motion.md`, `references/responsive.md` — core visual/system rules.
- `references/component-cookbook.md` and `references/components/` — reusable nav, hero, section, footer, and component patterns.
- `references/verbs/audit.md`, `references/verbs/redesign.md`, `references/study.md` — verb-specific workflows.
- `references/design-md.md` — DESIGN.md handoff and locking design DNA.
...
Hallmark is opinionated, short, and boring on purpose. It encodes a tight set of rules — drawn from the consensus of the anti-AI-slop design field (Anthropic's frontend-design skill, the AI design cookbook on frontend aesthetics, and the 2026 "tactile rebellion" movement) — and refuses to let the model fall back to the defaults every LLM was trained on.
...
**Source project by Hallmark contributors / Together AI; adapted locally for Hermes Agent.**
