...
description: "Use when creating or updating DESIGN.md from an existing product repository or public website by extracting evidence-based design language, tokens, and implementation guidance."
version: 0.1.0
...
- Source: `ibelick/ui-skills` original skill `create-design-md/SKILL.md`, commit `ae74b58`.
...
```bash
optional @google/design.md CLI: spec
```
...
```bash
optional @google/design.md CLI: lint DESIGN.md
```

Then run one compatibility export using the target selected above:

```bash
optional @google/design.md CLI: export --format <format> DESIGN.md
```
...
```bash
optional @google/design.md CLI: diff <previous-file> DESIGN.md
```
