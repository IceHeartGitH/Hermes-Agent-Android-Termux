...
description: Use when elite website image-to-code skill for Hermes Agent. For visually important web tasks, it must first generate the design image(s) itself, deeply analyze them, then implement the website to match them as closely as possible. In Hermes Agent, it must prefer large, readable, section-specific images instead of tiny compressed boards, generate fresh standalone images for sections or detail views instead of cropping old ones, avoid lazy under-generation, avoid cards-inside-cards-inside-cards UI, and keep the hero clean, spacious, readable, and visible on a small laptop.
version: 0.1.0
author: Leonxlnx / adapted for Hermes Agent
license: MIT
metadata:
  hermes:
    tags: [ui, ux, design, frontend, image]
    related_skills: []
    source: https://github.com/Leonxlnx/taste-skill/tree/main/skills/image-to-code-skill
...
- Source: `Leonxlnx/taste-skill/skills/image-to-code-skill/SKILL.md`.
...
- In Hermes Agent, bias strongly toward larger, more analyzable section images.
...
Inside Hermes Agent, do not compress too many website sections into one single image if that would make the text, spacing, buttons, or layout details too small to analyze properly.

In Hermes Agent, prefer separate large images per section.

Default rule inside Hermes Agent:
...
Outside Hermes Agent, this skill may still allow more compact multi-section composition when appropriate.
Inside Hermes Agent, prioritize section clarity and extraction accuracy.
...
When this skill is used inside Hermes Agent or any environment that supports image generation plus implementation, default to an image-first workflow for website design tasks.
...
Inside Hermes Agent, treat each section as its own analyzable unit.
...
In Hermes Agent, these should usually become section-by-section images, not one compressed sheet.
...
17. Has Hermes Agent avoided compressing too many sections into one tiny image?
...
4. inside Hermes Agent, prefer one large image per section
...
Do not compress many sections into one unreadable image in Hermes Agent.
...
Interpretation:
- generate 8 separate section images in Hermes Agent
- one per section
...
Interpretation:
- generate 4 separate section images in Hermes Agent
...
Inside Hermes Agent, if the user wants multiple sections, prefer separate large section images instead of one compressed multi-section board, so text, spacing, typography, buttons, and colors can be extracted properly.
