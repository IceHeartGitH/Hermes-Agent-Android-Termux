...
| `output_path` | string | no | Optional absolute path where the image should be saved (e.g. '/path/to/project/render.png'). If omitted, saved to /tmp/pisang_mcp/screenshots/. Use absolute paths — TD's working directory may differ from the agent's. |
