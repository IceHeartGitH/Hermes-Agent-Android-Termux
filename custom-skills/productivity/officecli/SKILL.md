---
name: officecli
description: "Use when working with OfficeCLI workflows. Create, analyze, proofread, and modify Office documents (.docx, .xlsx, .pptx) using the officecli CLI tool. Use when the user wants to create, inspect, check formatting, find issues, add charts, or modify Office documents. Hermes adaptation: do not auto-install OfficeCLI; verify Termux binary compatibility before running OfficeCLI commands."
version: 0.1.0
author: iOfficeAI / adapted for Hermes Agent
license: Apache-2.0
metadata:
  hermes:
    tags: [office, productivity, docx, xlsx, pptx, officecli]
    source: https://github.com/iOfficeAI/OfficeCLI
    source_commit: 7951552
    original_path: SKILL.md
    local_category: OfficeCLI Skills
---
...
- Source: `iOfficeAI/OfficeCLI`, commit `7951552`.
- Original upstream skill is preserved unchanged at `references/original-SKILL.md`.
- This active Hermes skill is a workflow/reference integration only unless the `officecli` binary has been separately verified on this Android/Termux host.
- Do not run upstream curl-pipe-shell, npm global install, MCP registration, or agent auto-install commands automatically.
- Before executing real OfficeCLI commands, check `command -v officecli` and `officecli --version`; if missing or incompatible, use existing Hermes productivity skills (`docx`, `xlsx`, `powerpoint`, `pdf`) or ask before testing a binary.
- Use OfficeCLI help commands when the binary is available; do not guess element/property syntax.
...
```bash
# macOS / Linux
download or install OfficeCLI only after explicit user approval and Termux compatibility verification

# Windows (PowerShell)
Windows PowerShell installer omitted in Hermes active skill
```
...
**Before doc work, check Specialized Skills** (bottom of this file). Fundraising decks, academic papers, financial models, dashboards, and Morph animations need their own skill loaded first — `load the corresponding Hermes skill` once, then proceed.
...
`officecli load the corresponding Hermes skill <name>` — output is a SKILL.md, follow its rules.
...
Example: a fundraising deck task → `officecli load the corresponding Hermes skill pitch-deck` → use the printed rules.
