...
3. Store in `${HERMES_HOME:-~/.hermes-venv}/.env`:
   ```
...
Add those exports to your shell profile (or to `${HERMES_HOME:-~/.hermes-venv}/.env`) so every session inherits them.
...
| `NOTION_KEYRING=0` | File-based creds at `Notion credential file` instead of OS keychain |
