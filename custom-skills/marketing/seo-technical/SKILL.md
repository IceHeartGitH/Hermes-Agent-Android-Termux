...
- Source: `AgriciDaniel/claude-seo/skills/seo-technical/SKILL.md`.
...
- XML sitemap: run `hermes-seo run sitemap_discovery.py <url> --json`; require a
...
hermes-seo run agent_ux_check.py https://example.com --json
```
...
without scoring, use `hermes-seo run render_page.py <url> --a11y-tree --json`.
...
If Google API credentials are configured, use `hermes-seo run pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `hermes-seo run crux_history.py <url> --json` for 25-week CWV trends, and `hermes-seo run gsc_inspect.py <url> --json` for real indexation status per URL.
