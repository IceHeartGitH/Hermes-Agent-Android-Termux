---
name: seo-audit
description: Use when full website SEO audit with parallel subagent delegation. Crawls up to 500 pages, detects business type, delegates to up to 15 specialists (8 always + 7 conditional), generates health score. Use when user says audit, full SEO check, analyze my site, or website health check.
version: 0.1.0
author: AgriciDaniel / adapted for Hermes Agent
license: MIT
metadata:
  hermes:
    tags: [marketing, seo]
    related_skills: [seo-backlinks, seo-cluster, seo-content, seo-dataforseo, seo-drift, seo-ecommerce, seo-geo, seo-google, seo-local, seo-maps, seo-schema, seo-sitemap]
    source: https://github.com/AgriciDaniel/claude-seo/tree/main/skills/seo-audit
    local_category: SEO/GEO
---
...
- Source: `AgriciDaniel/claude-seo/skills/seo-audit/SKILL.md`.
- Local category: SEO/GEO.
- Use this as a Hermes Agent SEO/GEO workflow skill. The upstream body below is adapted to remove Claude-specific installation/runtime assumptions.
- Original full file is stored at `references/original-SKILL.md`.
- Do not run upstream installers automatically on Termux. Treat heavy dependencies and external API credentials as optional, explicit setup steps.
...
1. **Render homepage**: use `hermes-seo run render_page.py <url> --mode auto --json` to capture raw HTML, rendered HTML, extracted text, SPA status, and accessibility data when needed
...
4. **Delegate to subagents** (if available, otherwise run inline sequentially):
   ...
   - `seo-google` -- CWV field data (CrUX), URL indexation (GSC), organic traffic (GA4) (spawn when Google API credentials detected via `hermes-seo run google_auth.py --check`)
   - `seo-backlinks` -- Backlink profile data: DA/PA, referring domains, anchor text, toxic links (spawn when Moz or Bing API credentials detected via `hermes-seo run backlinks_auth.py --check`, or always include Common Crawl domain-level metrics)
   ...
   - `seo-drift` -- Drift analysis: compare against stored baseline (spawn when drift baseline exists for the URL via `hermes-seo run drift_history.py <url>`)
...
- **PDF Report** (recommended): Generate a professional A4 PDF using `hermes-seo run google_report.py --type full --data {domain}-audit/audit-data.json --domain <domain> --output-dir {domain}-audit/`. This produces a white-cover enterprise report with TOC, executive summary, charts (Lighthouse gauges, query bars, index donut), metric cards, threshold tables, prioritized recommendations with effort estimates, and implementation roadmap. Always offer PDF generation after completing an audit.

## Structured Audit Data Envelope

Write `{domain}-audit/audit-data.json` with this shape so `hermes-seo run google_report.py --type full --data {domain}-audit/audit-data.json --domain <domain> --output-dir {domain}-audit/` can generate a report even when Google API data is unavailable:
